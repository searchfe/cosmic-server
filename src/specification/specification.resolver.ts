...
import { fileds2MongoProjection } from '@/common/util/db';
...
export class SpecificationResolver {
    ...
    async getAllSpecification(
        @Args({ name: 'teamId', type: () => String }) teamId: string,
    ) {
    ...
    async getCategory(
        @Args({ name: 'specificationId', type: () => String })
        ...
        const projection =
            fields && fields.length > 0
                ? fileds2MongoProjection(fields || [])
                : undefined;
        const result = await this.specificationService.getCategory(
            specificationId,
            { name: '字体' },
            projection,
        );
    ...
    async createSpecification(
        @Args('specification') specification: CreateSpecificationDTO,
    ) {
    ...
    async updateSpecification(
        ...
        const result = await this.specificationService.update(
            specification,
            projection,
        );
        return {
    ...
    async createCategory(@Args('category') category: CreateCategoryDTO) {
        ...
        return await this.specificationService.createCategory(
            category.specificationId,
            newCategory,
        );
    }
    ...
    async updateCategory(@Args('category') category: UpdateCategoryDTO) {
        ...
        delete newCategory.specificationId;
        return await this.specificationService.updateCategory(
            category.specificationId,
            newCategory,
        );
    }
    ...
    async deleteCategory(
        @Args('specificationId') specificationId: string,
        @Args('categoryId') categoryId: string,
    ) {
        return await this.specificationService.deleteCategory(
            specificationId,
            categoryId,
        );
    }
