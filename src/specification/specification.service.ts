...
export class SpecificationService {
    ...
    async findOne(
        ...
        fields?: MongoProjection<Specification>,
    ) {
        if (!fields) {
    ...
    async findAll(teamId: string) {
        const result = await this.specificationModel
            .where('team')
            .equals(Types.ObjectId(teamId))
            .lean()
            .exec();
    ...
    async getCategory(
        specificationId: string,
        categoryCondition: { [key in keyof Category]?: unknown },
        fields?: MongoProjection<Specification>,
    ) {
        ...
        const result = this.specificationModel
            .findById(specificationId)
            .select(projection);
        if (!fields) {
    ...
    async update(
        specification: PartialSpecification,
        fields?: MongoProjection<Specification>,
    ) {
        const result = this.specificationModel.findByIdAndUpdate(
            specification.id,
            specification,
            { new: true },
        );
    ...
    async createCategory(specificationId: string, category: Partial<Category>) {
        ...
        await this.specificationModel.findByIdAndUpdate(specificationId, {
    ...
    async updateCategory(
        specificationId: string,
        category: Pick<Category, 'name' | 'icon'> & { id: string },
    ) {
